...
def selection_cursor(day,hourS, hourE, month, pp):
    return_hours_days = "return this.properties.t_createdA.getHours()>" + str(hourS)+ " && this.properties.t_createdA.getHours()<=" + str(hourE)+ " && this.properties.t_createdA.getDay() == " + str(day)+" && this.properties.t_createdA.getMonth() == "+ str(month)
    print(return_hours_days)
    result_db = data.find(
        {"$where": return_hours_days})
    return result_db
#dayselection ()
...
def selection(day,hourS, hourE, month, pp):
    ...
    for r in result_db:
        ...
        result.append({"point": {"type": "Point", "coordinates": [lat, lng]}, "timeB": 0, "timeE": 0, "type": 0,
